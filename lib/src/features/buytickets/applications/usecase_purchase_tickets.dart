...
class UseCasePurchaseTickets implements IUseCase<RequestBodyPurchasedTickets, String> {
  final IRepositoryGameEvents repositoryGameEvents;

  UseCasePurchaseTickets({required this.repositoryGameEvents});

  @override
  Future<Either<Failure, String>> execute(RequestBodyPurchasedTickets request) async {
    ApiResponse response = await repositoryGameEvents.purchaseTickets(request);
    ...
    try{
      ...
      Debugger.debug(
        title: "UseCasePurchaseTickets(): parsing-error",
        data: e,
      );
    }
