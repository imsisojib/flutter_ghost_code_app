...
class UseCaseFetchProductsHoodie implements IUseCase<Empty, List<Product>> {
  final IRepositoryMerchandise repositoryMerchandise;

  UseCaseFetchProductsHoodie({required this.repositoryMerchandise});

  @override
  Future<Either<Failure, List<Product>>> execute(_) async {
    ApiResponse response = await repositoryMerchandise.fetchProductsByType("hoodie");
