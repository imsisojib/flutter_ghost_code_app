...
class UseCaseFetchProductsTShirt implements IUseCase<Empty, List<Product>> {
  final IRepositoryMerchandise repositoryMerchandise;

  UseCaseFetchProductsTShirt({required this.repositoryMerchandise});

  @override
  Future<Either<Failure, List<Product>>> execute(_) async {
    ApiResponse response = await repositoryMerchandise.fetchProductsByType("tshirt");
